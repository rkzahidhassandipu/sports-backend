...
const PAYMENT_INCLUDE = {
  booking: { include: { session: { select: { title: true, date: true, startTime: true } } } },
  user: { select: { id: true, name: true, email: true } },
};

export async function getMyPayments(userId: string, skip: number, take: number) {
  const [data, total] = await prisma.$transaction([
    prisma.payment.findMany({
      where: { userId },
      include: { booking: { include: { session: { select: { title: true, date: true } } } } },
...
export async function getPayment(id: string, userId: string, role: string) {
  ...
  if (p.userId !== userId && !["ADMIN", "RECEPTIONIST"].includes(role)) throw AppError.forbidden("Access denied");
  return p;
}

export async function getAllPayments(skip: number, take: number, filters: { status?: string; method?: string }) {
  const where: any = {};
  if (filters.status) where.status = filters.status;
  if (filters.method) where.method = filters.method;
  const [data, total] = await prisma.$transaction([
    prisma.payment.findMany({ where, include: PAYMENT_INCLUDE, orderBy: { createdAt: "desc" }, skip, take }),
...
export async function refundPayment(id: string, amount?: number) {
  ...
  if (!p) throw AppError.notFound("Payment not found");
  if (p.status !== "PAID") throw AppError.badRequest("Only paid payments can be refunded");
...
export async function getPaymentStats() {
  ...
  const [totalRev, monthRev, lastMonthRev, byMethod, recent] = await prisma.$transaction([
    ...
    prisma.payment.groupBy({ by: ["method"], where: { status: "PAID" }, _sum: { amount: true }, _count: { method: true } }),
    prisma.payment.findMany({ where: { status: "PAID" }, include: PAYMENT_INCLUDE, orderBy: { paidAt: "desc" }, take: 5 }),
  ]);
  ...
  return {
    ...
    revenueGrowth: lastMonthRev._sum.amount
      ? (((Number(monthRev._sum.amount) - Number(lastMonthRev._sum.amount)) / Number(lastMonthRev._sum.amount)) * 100).toFixed(1)
      : null,
    byMethod,
...
export async function generateInvoiceData(id: string, userId: string) {
  const p = await prisma.payment.findFirst({
    where: { id, userId },
    include: {
      booking: { include: { session: { include: { coach: { select: { name: true } } } } } },

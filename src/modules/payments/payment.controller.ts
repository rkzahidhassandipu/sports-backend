...
export const getPayment = asyncHandler(async (req: Request, res: Response) => {
  ApiResponse.success(res, await svc.getPayment(req.params.id, req.user!.id, req.user!.role));
});
...
export const refundPayment = asyncHandler(async (req: Request, res: Response) => {
  ...
  ApiResponse.success(res, await svc.refundPayment(req.params.id, amount), "Refund initiated");
});
...
export const downloadInvoice = asyncHandler(async (req: Request, res: Response) => {
  const invoice = await svc.generateInvoiceData(req.params.id, req.user!.id);

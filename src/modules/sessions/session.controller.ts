...
export const getSession = asyncHandler(async (req: Request, res: Response) => {
  ...
  ApiResponse.success(res, await svc.getSession(id));
});
...
export const updateSession = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  ApiResponse.success(res, await svc.updateSession(id, req.body, req.user!.id, req.user!.role), "Session updated");
});
...
export const uploadCover = asyncHandler(async (req: Request, res: Response) => {
  ...
  const id = req.params.id as string;
  ApiResponse.success(res, await svc.uploadCover(id, req.file.buffer), "Cover uploaded");
});
...
export const getSessionReviews = asyncHandler(async (req: Request, res: Response) => {
  const { page, limit, skip } = getPagination(req);
  const id = req.params.id as string;
  const result = await svc.getSessionReviews(id, skip, limit);
  ApiResponse.paginated(res, result.data, { page, limit, total: result.total }, "Reviews retrieved");
});

export const createReview = asyncHandler(async (req: Request, res: Response) => {
  const id = req.params.id as string;
  ApiResponse.created(res, await svc.createReview(id, req.user!.id, req.body), "Review submitted");
});
